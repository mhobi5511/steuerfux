...
export function calculateDueDate(issueDate: string, paymentTerm: string, customDueDate?: string) {
  ...
  if (paymentTerm === "7 Tage") date.setDate(date.getDate() + 7);
  else if (paymentTerm === "14 Tage") date.setDate(date.getDate() + 14);
  else if (paymentTerm === "30 Tage") date.setDate(date.getDate() + 30);
